...
public class TilePatternManager : MonoBehaviour
{
    #region static
    public static TilePatternManager GetInstance;
    private void Awake()
    {
        GetInstance = this;
    }
    ...
    public void VisibleTilePattern(GameObject obj)
    {
        ...
        var tilepattern = obj.GetComponent<TilePattern>();
        ...
        curTileColor = tilepattern.TilePatternColor;
    }

    /// <summary>
    /// Tile과 상호작용키를 통해 TilePattern Canvas를 연 상태에서
    /// 주사위의 값을 전달받는 함수.
    /// </summary>
    ...
    /// <summary>
    /// TilePattern Canvas가 오픈되어있지 않을 때, 플레이어가 인벤틀 클릭하여
    /// TilePattern에서 사용할 object를 꺼내려할때, 데이터 값만 이동시켜준다.
    /// 
    /// TilePattern Canvas가 오픈되어 있을 때, 플레이어가 인벤을 클릭하여 가져온
    /// 해당 오브젝트로 변경시킨다.
    /// </summary>
